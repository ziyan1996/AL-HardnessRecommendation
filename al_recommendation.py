...
#pool_X.set_option('display.max_columns', None)
# allows for the export of data to excel file
#header=gf.column_names
#header.insert(0,"Composition")
...
if selection == '1':
    ...
    rt_pred_y['rt_std']=rt_std
    rt_pred_y['rt_ypred']=rt_mean
    rt_pred_y['composition']=pool_compositions
    rt_pred_y['space_group']=pool_spacegroups
    rt_pred_y['temperature']=293.15
    #rt_sorted_pred=rt_pred_y.sort_values(by='std', ascending=False)
    #rt_tops = rt_sorted_pred.head(50)
    #print_rt_tops = rt_tops[['composition', 'space_group','temperature', 'ypred', 'std']].copy()
    #print_rt_tops.to_excel('query_by_committee_RT_recommendation.xlsx')
    ...
    ht_pred_y['ht_std']=ht_std
    ht_pred_y['ht_ypred']=ht_mean
    ht_pred_y['composition']=pool_compositions
    ht_pred_y['space_group']=pool_spacegroups
    ht_pred_y['temperature']=1073.15

    loss = 100*(rt_pred_y['rt_ypred'] - ht_pred_y['ht_ypred'])/rt_pred_y['rt_ypred']
    ht_pred_y['% loss']=loss
    ht_pred_y['rt_ypred']=rt_pred_y['rt_ypred']
    ht_pred_y['rt_std']=rt_pred_y['rt_std']
    rt_sorted_pred=ht_pred_y.sort_values(by='rt_std', ascending=False)
    rt_tops = rt_sorted_pred.head(150)
    print_rt_tops = rt_tops[['composition', 'space_group','rt_ypred', 'rt_std', 'ht_ypred', '% loss']].copy()
    print_rt_tops.to_excel('query_by_committee_RT_recommendation.xlsx')
    print('Query by committee \nRoom temperature (293.15K) recommendations: ')
    print(print_rt_tops)

    ht_sorted_pred=ht_pred_y.sort_values(by='ht_std', ascending=False)
    ht_tops = ht_sorted_pred.head(150)
    print_ht_tops = ht_tops[['composition', 'space_group','ht_ypred', 'ht_std', 'rt_ypred', '% loss']].copy()
    print_ht_tops.to_excel('query_by_committee_HT_recommendation.xlsx')
    ...
    #rt_tops = sorted_pred.head(50)
    #print_rt_tops = rt_tops[['composition', 'space_group', 'temperature','ypred', 'std']].copy()
    #print_rt_tops.to_excel('Bagging_RT_recommendation.xlsx')
    ...
    rt_tops = rt_sorted_pred.head(150)
    print_rt_tops = rt_tops[['composition', 'space_group','rt_ypred', 'rt_std', 'ht_ypred', '% loss']].copy()
    print_rt_tops.to_excel('query_by_bagging_RT_recommendation.xlsx')
    print('Query by bagging \nRoom temperature (293.15K) recommendations: ')
    print(print_rt_tops)

    ht_sorted_pred=ht_pred_y.sort_values(by='ht_std', ascending=False)
    ht_tops = ht_sorted_pred.head(150)
    print_ht_tops = ht_tops[['composition', 'space_group','ht_ypred', 'ht_std', 'rt_ypred', '% loss']].copy()
    print_ht_tops.to_excel('query_by_bagging_HT_recommendation.xlsx')
    print('Query by bagging \nHigh temperature (1073.15K) recommendations: ')
    print(print_ht_tops)
    ...
    #rt_tops = sorted_pred.head(50)
    #print_rt_tops = rt_tops[['composition', 'space_group','temperature', 'ypred', 'std']].copy()
    #print_rt_tops.to_excel('AMD_RT_recommendation.xlsx')
    ...
    ht_tops = ht_sorted_pred.head(150)
    print_ht_tops = ht_tops[['composition', 'space_group','ht_ypred', 'ht_std', 'rt_ypred', '% loss']].copy()
    print_ht_tops.to_excel('AMD_HT_recommendation.xlsx')
    print('Adaptive max disagreement \nHigh temperature (1073.15K) recommendations: ')
    print(print_ht_tops)
